...
昨日の記事は、akaakuさんによる[ココパズルのすべて](https://blog.akaaku.net/posts/advent-calendar-2025-12-06)でした！
...
3. **GUIアプリのインストール**: Wingetを使ってGit、WezTerm、Visual Studio Code、Unity Hub、TigerVNC、Discord、Brave BrowserなどのGUIアプリをインストールします。Wingetを使ってGitをインストールしてるのは、他のプログラムでProgram FilesにインストールされているGitを参照してしまう問題を避けるためです。[^1]
...
もしgitがない場合は先に`winget install Git.Git`などでインストールしてください。

もしくはこのセットアップスクリプトをコピーしてテキストファイルに貼り付け、`setup-windows.ps1`などの名前で保存しても構いません。
...
[^1]: [Windows向けパッケージマネージャーの比較](https://note.com/zephel01/n/n3d3cd1f665b4)
